...
import software.amazon.awssdk.retries.internal.ratelimiter.DefaultRateLimiterTokenBucket;
import software.amazon.awssdk.retries.internal.ratelimiter.DefaultRateLimiterUpdateResponse;
import software.amazon.awssdk.retries.internal.ratelimiter.RateLimiterTokenBucketStore;

public class DefaultRateLimiterTokenBucketTest {
    private static MutableClock clock = null;
    private static DefaultRateLimiterTokenBucket tokenBucket = null;
    private static final double EPSILON = 0.0001;

    @BeforeAll
    static void setup() {
        clock = new MutableClock();
        tokenBucket = new DefaultRateLimiterTokenBucket(clock, false);
    }
    ...
    public void testCase(TestCase testCase) {
        ...
        DefaultRateLimiterUpdateResponse res;
        tokenBucket.tryAcquire(1);
        if (testCase.throttleResponse) {
            res = (DefaultRateLimiterUpdateResponse) tokenBucket.updateRateAfterThrottling();
        } else {
            res = (DefaultRateLimiterUpdateResponse) tokenBucket.updateRateAfterSuccess();
        }
        double measuredTxRate = res.after().measuredTxRate();
        assertThat(measuredTxRate).isCloseTo(testCase.expectMeasuredTxRate, within(EPSILON));
        double fillRate = res.after().fillRate();
